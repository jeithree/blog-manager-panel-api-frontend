...
import {CreatePostData, postService, PostStatus} from '@/services/post';
...
export default function CreatePostPage() {
	...
	const saveTags = useCallback(async () => {
		...
		// Only map generated tag names to existing tags; do not create new tags here.
		const matchedIds = generatedTags
			.map((name) =>
		...
		// Replace previous auto-added tags with newly matched ones while preserving manual tags
		setSelectedTagIds((prev) => {
			// remove previously auto-added tag ids
			const withoutAuto = prev.filter((id) => !autoAddedTagIds.includes(id));
			const combined = [...withoutAuto, ...matchedIds];
			return Array.from(new Set(combined));
		});

		// remember which tags we auto-added so they can be replaced next time
		setAutoAddedTagIds(matchedIds);

		// Clear generated tags after processing
		setGeneratedTags([]);
	}, [generatedTags, tags, autoAddedTagIds]);
	...
	const generateContent = async () => {
		...
		setIsLoading(true);
		try {
			// pass slug generated by title suggestion (or derive from title if missing)
			const finalSlug =
				slug ||
				getSelectedTitle()
					.toLowerCase()
	...
	const handlePublish = async (status: PostStatus) => {
		// For drafts allow minimal required fields (blog + title).
		if (status === PostStatus.DRAFT) {
			...
			// For non-draft statuses require all necessary publishing fields
			if (
				!selectedBlogId ||
				!getSelectedTitle() ||
				!slug ||
		...
		setIsLoading(true);
		try {
			// Build post data only including optional fields when present
					...
					<div className="space-y-6">
						...
						{titleSuggestions.map((categoryGroup, groupIdx) => (
							<div
								...
								{categoryGroup.titles.map((titleObj, idx) => (
									<div
										key={idx}
										...
										<div className="flex items-center justify-between gap-4">
											<div
												className="flex-1"
												onClick={() => {
													setSelectedTitle(titleObj.title);
													setSlug(titleObj.slug);
